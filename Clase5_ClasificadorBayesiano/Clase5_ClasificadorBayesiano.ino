...
void bayesiano (int etiquetas, int columnas, int filas, float r) {
  ...
  */
  ...
  pX = list.Count() / float(filas);
  for (i = 0; i < etiquetas; i++) {
    prob[3][i] = (prob[1][i] / filas) * (prob[2][i] / prob[1][i]) / pX;
  }
  ...
  for (i = 0; i < etiquetas; i++) {
    for (j = i + 1; j < etiquetas; j++) {
      for (k = j + 1; k < etiquetas; k++) {
        if (prob[3][i] > prob[3][j] && prob[3][i] > prob[3][k])
          res=1;
          else if (prob[3][i] < prob[3][j] && prob[3][k] < prob[3][j])
          res=2;
